...
PlotFlower <- function(region_plot     = NA,
  ...
  conf <-  readr::read_csv(here('region/conf/goals.csv'))
  ...
  region_names_all <- bind_rows(
    ...
    read_csv(here('region/spatial/regions_list.csv')) %>%
  ...
  ## write out filenames
  readr::write_csv(region_names_all, file.path(here('region/reports/figures/regions_figs.csv')))
  ...
  for (region in region_plots) { # region = 301
    ...
    plot_obj <- plot_obj +
      ## add supragoal arcs
      geom_errorbar(data = supra_df, inherit.aes = FALSE,
                    aes(x = 0.5, ymin = supra_rad, ymax = supra_rad),
                    size = 0.25, show.legend = NA) +
      geom_text(data = supra_df, inherit.aes = FALSE,
                aes(label = name_supra, x = pos_supra, y = supra_rad, angle = myAng),
                hjust = .5, vjust = .5,
