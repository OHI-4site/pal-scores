...
The habitat subgoal measures the average condition of habitats present on the island that provide critical habitat for a broad range of species. This subgoal is considered a proxy for the condition of the broad suite of species on Palmyra. The atoll currently supports one of the best remaining coastal strand forests in the world (https://www.islandconservation.org/palmyra-atoll/). Given the importance of this ecosystem, both terrestrial and marine habitats are included in this assessment.   
...
The following zones were grouped together to form the three reef zones of interest:  
...
The benthic habitat mapping study included estimates of live coral cover across the different zones of the reef. Using the groupings above, the total number of observations in each zone was: 
...
Coral reef status is calculated as the number of data points falling within or above the reference point coral cover range compared to the overall number of data points in each zone of the reef.   

$status~=~ref_{n}~/~total_{n}$   

Where $ref_{n}$ is the number of data points in each zone falling within or above the reference point coral cover range and $total_{n}$ is the total number of data points in each zone. The area weighted mean is used to find the overall coral reef habitat status.     

*Rainforests*   

The rainforest condition is based on the land area of the rainforest. Invasive coconut palms (*Cocos nucifer*) are limiting the land area that can be occupied by native rainforests. Palmyra has planned coconut palm removal activities, which they hope will increase the land area of native forests. 

The raw data included four vegetation categories native forests, *Scaevola*, non-vegetated areas, and *Cocos nucifer* (coconut palm). The native forest and *Scaevola* categories were grouped together to find the total forest land area. Non-vegetated areas were excluded from the analysis. 
...
Data to calculate the marine debris layer comes from a table of Fishing Aggregation Device (FAD) found ashore on Palmyra Atoll. Data were obtained from Alex Wegmann at The Nature Conservancy on July 8, 2020.   
...
Data for the carbon storage status comes from tree cover surveys completed as part of the U.S. Geological Survey vegetation community mapping project for Palmyra Atoll. Vegetation maps were completed in October 2016 and published in June 2019. The data is available for download [here](https://www.sciencebase.gov/catalog/item/5c66fc5be4b0fe48cb3aca49).    
...
The iconic species trend is calculated as a linear change in iconic species status.     
...
The trend for lasting special places is calculated as the linear change in percent protected areas over time. 

**Data**

Data was obtained verbally from Alex Wegmann at The Nature Conservancy on June 29, 2020. 
