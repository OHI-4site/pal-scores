...
Original raw data included 10 different zones, as shown in the map on the left-hand side. The following zones were grouped together to form the four reef zones of interest, as shown on the map on the right-hand side:    
...
The species fall within 10 different taxonomic classes. Since there are not an even number of species in each class, the status is first calculated for each class as the average of the extinction risk scores. The overall status is then found by averaging the extinction risk score of all 10 classes. Finally, the score is rescaled so a risk factor of 0.75 is equal to zero, as in the following equation. 

$status~=~(0.75~-~mean~risk)~/~0.75$   
