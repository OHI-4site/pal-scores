...
<center>
![coral_zones_raw](images/coral_zones_raw.png){width=47%} ![coral_zones_reclass](images/coral_zones_reclassified.png){width=47%}  
</center>
***Figure 4.1:** Map A (left) displays coral zones as shown in the raw data and map B (right) shows the coral zones as they have been reclassified for this assessment.*      
...
$$
status_{zone}~=~ref_{area}~/~total_{area},  ~~(Eq~4.1) 
$$  

Where $ref_{area}$ is the area of the reef zone falling within or above the reference point category and $total_{area}$ is the total area of the reef zone. For the status calculation we adjust the area of the lagoon to include only the area of the lagoon where coral can reasonably grow. Other zones are not area adjusted due to incomplete knowledge and data. In the future adjusting the area of all zones to include only the area where coral can be expected to grow would strengthen this analysis.          
...
$$
status = area_{current}~(km^2)~/~area_{potential}~(km^2), ~~(Eq~ 4.2) 
$$  
...
$$
area_{potential}~=~area_{added}~(km^2)~+~area_{original}~(km^2) , ~~(Eq 4.3)
$$

And where   

$$
area_{added}~=~0.95~*~(0.43~*~2.46~(km^2)), ~~(Eq~ 4.4)
$$   
$$
area_{original}~=~0.482~*~2.46~(km^2), ~~(Eq~4.5)
$$   
...
$$
status~=~(0.75~-~mean~risk)~/~0.75, ~~(Eq ~4.6)
$$   

Where $mean~risk$ is the average species extinction risk score, found by averaging the mean extinction risk score of all 10 classes. 
...
$$
status~=~(fad_{y}~-~fad_{reference})/~fad_{reference} ~~(Eq ~4.7)
$$   
...
**Table 4.2. Threshold Effect Levels (TELs) and Ecological soil screening levels (Eco-SSLs) for the main contaminants found in soils around Palmyra. NA indicates a toxicity threshold has not been established.**        
...
**Table 4.3. Freshwater contamination Probable Effect Levels (PELs) for the main contaminants found in soils around Palmyra.**   
...
**Table 4.4. Marine sediment Probable Effect Levels (PELs) for the main contaminants found in soils around Palmyra.**   
...
A status is calculated for each site using the geometric mean of the 9 contaminants (Equation 4.8). An overall status is then found using an area weighted mean of the site status (Equation 4.9). This gives high scores to years with greater levels of contamination. This status is used as a pressure layer. The final status layer for the Clean Waters goal is the inverse of this status score, found using 1 - status.  

$$ Geometric~mean ~~(Eq ~4.8) 
$$   

$$ Overall~status ~~(Eq ~4.9)
$$   
<br>   
**Trend**   
...
Current status is calculated as the current percent of forest dominated by native trees over the reference point percentage (Equation 4.10).    

$$
status~=~percent~cover~/~reference~point~percent~cover  ~~(Eq ~4.10)
$$
<br>   
**Trend**   
...
$$
status~=~(C_{condition}~*~A~*~w)~/~(A~*~w)  ~~(Eq ~4.11) 
$$   
...
The employment layer is calculated based on the number of full time equivalent (FTE) positions in conservation science supported by The Nature Conservancy. The reference point for employment is five FTE positions. Status scores are re-scaled to 0-1 by dividing the number of FTE positions by the reference point (Equation 4.12).    

$$
status = FTE_{y}~/~ref ~~(Eq ~4.12)
$$  
...
$$
status = publications_{y}~/~ref  ~~(Eq ~4.13)
$$  
...
The reference point is 100% of species have a status of "Least Concern". The status is found by averaging the IUCN category score for each of the species on the iconic species list (Equation 4.14).   
...
$$
{ p }\quad =\quad \gamma *{ p }_{ E }\quad +\quad (1-\gamma )*{ p }_{ S } ~~(Eq.~4.17)
$$
...
We determined the rank sensitivity of each goal/sub-goal to each stressor (or, when possible, an element of the goal, such as a specific habitat). We ranked ecological pressures as having ‘high’ (score = 3), ‘medium’ (score = 2), ‘low’ (score = 1), or ‘no’ (score = NA) impact (Table 4.8). Wherever possible we relied on peer-reviewed literature to establish these rankings, and relied on our collective expert judgment in cases with no available literature (Table S28 in Halpern *et al.* 2012). When possible Palmyra specific estimates were made and where no better estimates existed, the global rankings were used.  
<br>   
...
$$
{ p }_{ i }\quad =\quad \frac { \displaystyle\sum _{ i=1 }^{ N }{ { w }_{ i }{ s }_{ i } }  }{ 3 }, ~~(Eq. ~4.18)
$$
...
$$
{ p }_{ E }\quad =\quad \frac { \displaystyle\sum _{ i=1 }^{ N }{ { (w }_{ i\_ max }*{ p }_{ i }) }  }{ \displaystyle\sum_{ i=1 }^{ N } { { w }_{ i\_ max } }  },  ~~(Eq. ~4.19)
$$
...
Social pressures describe the lack of effectiveness of government and social institutions. For Palmyra, the social pressure we are considering is management. Social stressors are described for each region on a scale of 0 to 1 (with one indicating the highest pressure). Social pressure is then calculated as the average of the social stressors:

$$
{ p }_{ S }\quad =\quad \frac { \displaystyle\sum _{ i=1 }^{ N }{ z_{ i } }  }{ N }, ~~(Eq. ~4.20)
$$
...
| Measure of climate change management | res_cc_mgmt        | ecological  | ecosystem     | regulatory  | 1 |
| Measure of fishing (offshore) regulations | res_fp_reg    | ecological  | ecosystem     | regulatory  | 0.5 |
| Measure of fishing (offshore) management  | res_fp_mgmt   | ecological  | ecosystem     | regulatory  | 0.5 |
| Measure of habitat destruction regulations | res_hd_reg   | ecological  | ecosystem     | regulatory  | 0.5 |
| Measure of terrestrial and marine habitat management  | res_hd_mgmt   | ecological  | ecosystem     | regulatory  | 0.5 |
| Measure of alien species regulations | res_sp_reg    | ecological  | ecosystem     | regulatory  | 0.5 |
| Measure of alien species management  | res_sp_mgmt   | ecological  | ecosystem     | regulatory  | 0.5 |
<br>   
