...
Contaminants are scaled 0-1 using a midpoint level, (TEL or ECO-SSl) and an upper level (PELs) (Equation 4.8). A status is then calculated for each site using the geometric mean of the 9 contaminants (Equation 4.9).  

$$
Scaling~equation ~~(Eq ~4.8)
$$ 
<br>
$$
Geometric~mean ~~(Eq ~4.9) 
$$   
Finally, an overall status is found using an area weighted mean of the site status (Equation 4.10). This gives high scores to years with greater levels of contamination. This status is used as a pressure layer. The final status layer for the Clean Waters goal is the inverse of this status score, found using 1 - status.   

$$ 
Overall~status ~~(Eq ~4.10)
$$
<br>   
**Trend**   
...
Current status is calculated as the current percent of forest dominated by native trees in year (y) over the reference point percentage (Equation 4.11).    

$$
status~=~ \frac {percent~cover_{y} } {percent~cover_{ref} }~,  ~~~(Eq ~4.11)
$$
<br>   
**Trend**   
...
The Coastal Protection status is calculated as the relative health of the habitats that provide coastal protection weighted by their area and protectiveness rank (Equation 4.12).     

$$
status~=~ \frac { (C~*~A~*~w) } { (A~*~w) }~, ~~~(Eq ~4.12) 
$$   
...
The employment layer is calculated based on the number of full time equivalent (FTE) positions in conservation science supported by The Nature Conservancy. The reference point for employment is five FTE positions. Status scores are re-scaled to 0-1 by dividing the number of FTE positions by the reference point (Equation 4.13). Any value over one is given a score of 1.     

$$
status~=~ \frac { FTE_{y} } { FTE_{ref} }, ~~(Eq ~4.13)
$$  
...
The publications layer is calculated based on the number of scientific studies published about resources on Palmyra each year. The reference point is 10 annual publications. Status scores are re-scaled to 0-1 by dividing the number of scientific publications by the reference point (Equation 4.14).  

$$
status~=~ \frac { pubs_{y} } { pubs_{ref} },  ~~(Eq ~4.14)
$$  
...
**Update method once the 'zero' score fishing amount is determined**.  
...
The reference point is 100% of species have a status of "Least Concern". The status is found by averaging the IUCN category score for each of the species on the iconic species list (Equation 4.16).   
...
$$
status_{offshore}~=~ \frac { pa_{offshore} } { pa_{offshore, ref} }, ~~(Eq ~4.17)
$$   
<br>    
$$
status_{inland}~=~ \frac { pa_{inland}} { pa_{inland, ref} },  ~~(Eq ~4.18)
$$    
...
**Update methods once 'zero' tourism number is determined**   
...
**Insert description of regulatory resilience layers - maybe delete the next paragraph**. 
