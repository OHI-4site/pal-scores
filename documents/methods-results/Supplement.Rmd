...
Three zones were used to calculate the coral reef status: back reef, fore reef, and lagoon. 
...
Where $ref_{n}$ is the number of data points in each zone falling within or above the reference point coral cover range and $total_{n}$ is the total number of data points in each zone.   
...
The trend in rainforest condition is based on a linear model to calculate the rate of change in forest area per year using data from 2010-2019. The trend is the five year average change in rainforest area.   
...
The species trend is calculated using data the IUCN provides for current and past assessments of species, which we use to estimate annual change in IUCN risk status for each species. 
